 ...
 * @constructor
 */
export var FFMpeg = function (options) {
  if (options.input) {
  ...
  this.quality =
    options.quality === undefined || options.quality === ''
      ? 3
      : options.quality;
...
FFMpeg.prototype._args = function () {
  return this.arguments.concat(
    ['-rtsp_transport', 'tcp', '-loglevel', 'fatal', '-i', this.input, '-r', this.rate.toString()],
...
FFMpeg.prototype.start = function () {
  ...
  this.child.stdout.on('data', function (data) {
	...
	this.child.stderr.on('data', function (data) {
	  console.log('this.child.stderr.on', data);
  ...
  this.child.on('error', function (err) {
    ...
    if (err.code === 'ENOENT') {
      throw new Error(
        "FFMpeg executable wasn't found. Install this package and check FFMpeg.cmd property",
      );
    } else {
      throw err;
    }
